...
- OpenAPI route metadata, schema-crate-neutral document generation from resolved
  ...
  documentation, request/response examples, and optional `serve_openapi(...)`
  JSON route registration plus `serve_openapi_ui(...)` Swagger UI route
  registration.
...
- Add Nest-style OpenAPI macros such as `#[tag]`, `#[operation]`,
  ...
  request/response examples, and auth metadata attributes. (Implemented)
...
- The generated document includes paths, methods, inferred and explicit params,
  request body, responses, request/response examples, tags, and security
  metadata.
