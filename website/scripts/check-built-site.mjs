...
const base = (process.env.DOCS_BASE ?? '/Boot/').replace(/^\/+|\/+$/g, '');
const locales = ['zh', 'en'];
...
for (const asset of ['a3s-boot-mark.svg', 'favicon.svg', 'social-card.svg']) {
...
assertIncludes(defaultHome, 'data-boot-home', 'custom Boot homepage');
assertIncludes(defaultHome, '简体中文', 'Chinese locale option');
assertIncludes(defaultHome, 'English', 'English locale option');
assertIncludes(defaultHome, 'aria-label="协议入口"', 'protocol tab list label');
assertIncludes(
  defaultHome,
  'aria-controls="boot-demo-panel"',
  'protocol tab panel relationship',
);
assertIncludes(defaultHome, 'role="tabpanel"', 'protocol tab panel semantics');
for (const version of versions) {
...
assertIncludes(englishHome, 'Modular applications', 'English homepage copy');
...
assertIncludes(
  nestedPage,
  `/${base}/v0.1.4/getting-started/overview`,
